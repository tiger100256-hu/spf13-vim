...
"    call Decho("setl ma noro",'~'.expand("<slnum>"))
    ...
    "unsilent echomsg "s:user".s:user
    "unsilent echomsg "s:machine".s:machine
    "unsilent echomsg "url".url
...
"  call Dfunc("s:RemotePathAnalysis(a:dirname<".a:dirname.">)")
  ...
  "unsilent echomsg "s:user".s:user
  "unsilent echomsg "s:method".s:method
  "unsilent echomsg "s:machine".s:machine
  if s:machine =~ '@'
   let dirpat    = '^\(.*\)@\(.\{-}\)$'
   if s:user != ""
    ...
    let s:user    = substitute(s:machine,dirpat,'\1','')
   endif
